...
const Strassen_1 = require("../Strassen");
const Matrix_1 = require("../Matrix");
let expect = chai.expect;
describe("description", () => {
    it("check decompos", () => {
        let x = [[0, 2], [5, 3]];
        expect(Strassen_1.Strassen.decompose(x).augmented).to.equal(false);
        x = [[0, 2, 3, 4], [5, 3, 0, 6], [1, 7, 4, 1], [0, 4, 3, 2]];
        expect(Strassen_1.Strassen.decompose(x).augmented).to.equal(false);
        x = [[0, 2, 3, 4, 6, 5], [5, 3, 0, 6, 1, 4], [1, 7, 4, 1, 8, 4], [0, 4, 3, 2, 0, 0], [0, 4, 3, 2, 0, 0], [0, 4, 3, 2, 0, 0]];
        expect(Strassen_1.Strassen.decompose(x).augmented).to.equal(false);
        x = [[0, 2, 4], [5, 3, 1], [5, 3, 2]];
        expect(Strassen_1.Strassen.decompose(x).augmented).to.equal(true);
    ...
    it("check decompos 4", () => {
        ...
        const dec = Strassen_1.Strassen.decompose(x);
        const join = Strassen_1.Strassen.join(dec);
    ...
    it("check mult", () => {
        const x = [[0, 2], [5, 3]];
        const y = [[-1, 2], [2, 0]];
        const m = Strassen_1.Strassen.multiply(x, y);
    ...
    const checkN = (n) => {
        ...
        const standard = Matrix_1.standardMult(a, b);
        const strassen = Strassen_1.Strassen.multiply(a, b);
