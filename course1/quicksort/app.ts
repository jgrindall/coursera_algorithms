import {QuickSort, Comparator, PivotCalc, LeftPivot, RightPivot, NumericMedianPivot} from "./QuickSort";
import fs from 'fs';
...
const sort = (data:string, pivotCalc:PivotCalc<number>)=>{
    const arr = data.split('\n').map((i: string) => parseInt(i));
    let qs = new QuickSort<number>(arr, comp);
    qs.setPivotCalc(pivotCalc);
    qs.sort(0, arr.length - 1);
    console.log(qs.numComp);
    for(let i = 0; i < arr.length; i++){
        console.assert(i + 1 === arr[i]);
    }
};
...
fs.readFile('input.txt', 'utf8', (err:any, data:string) => {
    ...
    sort(data, new LeftPivot<number>());
    sort(data, new RightPivot<number>());
    sort(data, new NumericMedianPivot());
});
...
recursive call on a subarray of length m, you should simply add
m−1 to your running total of comparisons. (This is because the
pivot element is compared to each of the other m-1 elements in the
...
1 point Enter answer here 2. Question 2 GENERAL DIRECTIONS AND HOW TO
GIVE US YOUR ANSWER:
...
1 point Enter answer here 3. Question 3 GENERAL DIRECTIONS AND HOW TO
GIVE US YOUR ANSWER:
...
length 2k, use the k^{th} element as the "middle" element. So for
the array 4 5 6 7, the "middle" element is the second one ---- 5 and not
...
simply add m-1m−1 to your running total of comparisons every time you
recurse on a subarray with length mm.
