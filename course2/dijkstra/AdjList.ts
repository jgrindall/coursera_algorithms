...
export type WeightedEdge = [string, number];
...
export class AdjList{
    ...
    getWeight(node0:string, node1:string):number{
        const edge:WeightedEdge = this.hash[node0].find(edge => edge[0] === node1);
        return edge[1];
    }
    hasEdge(node0:string, node1:string):boolean{
        ...
        return !!this.hash[node0].find(edge => edge[0] === node1);
    }
    getNeighbours(a:string):Array<string>{
        return this.hash[a].map((e:WeightedEdge) => e[0]);
    }
    addEdge(node0:string, node1:string, length:number):void{
        if(!this.hasEdge(node0, node1)){
            this.hash[node0].push([node1, length]);
        }
    }
    toString():string{
